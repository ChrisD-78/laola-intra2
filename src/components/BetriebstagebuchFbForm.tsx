...
type WaterPoolKeyFb = 'schwimmer' | 'nichtschwimmer' | 'plansch' | 'kleinkinder'

const FB_POOLS: Array<{ key: WaterPoolKeyFb; label: string }> = [
...
const HISTORY_POOLS = FB_POOLS.map((p) => ({ scope: 'freibad' as const, key: p.key, label: p.label }))
...
export interface BetriebstagebuchFbData {
  datum: string
  wochentag: string
  ...
  wasserwerteFreibad: Record<WaterPoolKeyFb, WaterSection>
...
interface BetriebstagebuchFbFormProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (data: BetriebstagebuchFbData) => void
  submissions: Array<{
    id: string
    submittedAt: string
    formData: Partial<BetriebstagebuchFbData>
    submittedBy?: string
  }>
}

export default function BetriebstagebuchFbForm({
  isOpen,
  onClose,
  onSubmit,
  submissions,
}: BetriebstagebuchFbFormProps) {
  ...
  const [personal, setPersonal] = useState<BetriebstagebuchFbData['personal']>({
    frueh: {
      schichtfuehrung: '',
      aufsicht2: '',
      aufsicht3: '',
  ...
  const [wasserwerteFreibad, setWasserwerteFreibad] =
    useState<Record<WaterPoolKeyFb, WaterSection>>(() => makePoolMap(FB_POOLS))
  ...
  const [kontrollgang, setKontrollgang] = useState<BetriebstagebuchFbData['kontrollgang']>({
    uhrzeit: '',
    handzeichen: '',
  })
  ...
  const [filterSpuelung, setFilterSpuelung] = useState<BetriebstagebuchFbData['filterSpuelung']>({
    filter1: false,
    filter2: false,
    filter3: false,
    filter4: false,
  ...
  const [unterschrift, setUnterschrift] = useState<BetriebstagebuchFbData['unterschrift']>({
  ...
  const resetForm = () => {
    ...
    setWasserwerteFreibad(makePoolMap(FB_POOLS))
    setSaeurekapazitaet('')
    ...
    setBehobenVon('')
    setBehobenUm('')
    setFilterSpuelung({ filter1: false, filter2: false, filter3: false, filter4: false })
    setSonstiges('')
  ...
  const updatePersonal = (shift: Shift, key: keyof BetriebstagebuchFbData['personal']['frueh'], value: string) => {
    setPersonal((prev) => ({
      ...prev,
  ...
  const historyWaterChartData = useMemo(() => {
    ...
    const [scope, key] = historySelectedPoolKey.split(':') as ['freibad', string]
    if (scope !== 'freibad') return []
    const SLOT: TimeSlot = 'betriebsbeginn'
    return historyRows
      .slice(0, 5)
      .reverse()
      .map((r) => {
        const data = r.data as Partial<BetriebstagebuchFbData>
        const raw = data?.wasserwerteFreibad?.[key as WaterPoolKeyFb]?.[SLOT]
        const section: WaterRow = raw ? recalcWaterRow(raw as WaterRow) : emptyWaterRow()
  ...
  const loadHistoryEntryIntoForm = (entry: {
    data: Partial<BetriebstagebuchFbData>
  }) => {
    ...
    setWasserwerteFreibad(
      normalizeAndRecalcPoolMap(
        makePoolMap(FB_POOLS),
        data.wasserwerteFreibad as Partial<Record<WaterPoolKeyFb, WaterSection>> | undefined,
        FB_POOLS.map((p) => p.key),
      ),
    )
    ...
    setFilterSpuelung({
      filter1: !!data.filterSpuelung?.filter1,
      filter2: !!data.filterSpuelung?.filter2,
      filter3: !!data.filterSpuelung?.filter3,
      filter4: !!data.filterSpuelung?.filter4,
    })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const poolKeys = FB_POOLS.map((p) => p.key)
    const payload: BetriebstagebuchFbData = {
      datum,
      wochentag,
      personal,
      wasserwerteFreibad: recalcEntirePoolMap(wasserwerteFreibad, poolKeys),
      montag: {
        saeurekapazitaet,
        messwasserentnahmestellenReinigen,
        kuvetteAustauschen,
      },
      lufttemperatur: { aussen: luftAussen },
      kontrollgang,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-0 sm:p-4 z-50">
      <div className="bg-white rounded-none sm:rounded-2xl shadow-xl w-screen sm:w-[95vw] max-w-none max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-gray-900">Betriebstagebuch FB</h2>
            <p className="text-sm text-gray-600">Digitales Formular nach FZB-AF-7.2.1.2-1 · Freibad</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
                          ...
                          <div key={field}>
                            ...
                            {field === 'clGeb' ? (
                              <input
                                readOnly
                                tabIndex={-1}
                                value={wasserwerteFreibad[pool.key][selectedTimeSlot].clGeb}
                              ...
                              <input
                                value={wasserwerteFreibad[pool.key][selectedTimeSlot][field]}
                                onChange={(e) =>
                                  updateWater(
                                    setWasserwerteFreibad,
                                    pool.key,
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="border border-gray-200 rounded-xl p-4">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Lufttemperatur (Außen)</h3>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Außen (°C)</label>
                <input
                  value={luftAussen}
                  onChange={(e) => setLuftAussen(e.target.value)}
                  className="w-full max-w-xs px-3 py-2 border border-gray-300 rounded-lg text-gray-900"
                />
              </div>
            </div>
          </div>
          ...
          <div className="border border-gray-200 rounded-xl p-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              Filterspülung durchgeführt (zum Schichtwechsel eintragen)
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {(
                ...
                <label key={k} className="flex items-center gap-2 text-sm text-gray-800">
                  <input
                    type="checkbox"
                    checked={filterSpuelung[k]}
                    onChange={(e) => setFilterSpuelung((prev) => ({ ...prev, [k]: e.target.checked }))}
                  />
                  {label}
                </label>
              ))}
        ...
        <>
          ...
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-0 sm:p-4">
            <div className="bg-white rounded-none sm:rounded-2xl shadow-2xl w-screen sm:w-[95vw] max-w-none max-h-[90vh] overflow-hidden flex flex-col">
              <div className="p-6 border-b border-gray-200 flex items-center justify-between">
                <h3 className="text-xl font-bold text-gray-900">Betriebstagebuch FB – Historie</h3>
                <button
                  type="button"
                ...
                {selectedHistory && (
                  <div ref={detailsSectionRef} className="border border-gray-200 rounded-xl p-4">
                    ...
                    <div className="space-y-6">
                      ...
                      {/* Wasserwerte Freibad */}
                      <div>
                        <h5 className="text-sm font-semibold text-gray-900 mb-2">Wasserwerte – Freibad</h5>
                        <div className="space-y-4">
                          {FB_POOLS.map((p) => {
                            const sec = selectedHistory.data?.wasserwerteFreibad?.[p.key]
                          ...
                          {FB_POOLS.every((p) => !hasAnyWaterValue(selectedHistory.data?.wasserwerteFreibad?.[p.key])) && (
                      ...
                      {/* Filterspülung */}
                      <div>
                        <h5 className="text-sm font-semibold text-gray-900 mb-2">Filterspülung durchgeführt</h5>
                        {renderKeyValueTable([
                          { label: 'Filter 1', value: selectedHistory.data?.filterSpuelung?.filter1 },
                          { label: 'Filter 2', value: selectedHistory.data?.filterSpuelung?.filter2 },
                          { label: 'Filter 3', value: selectedHistory.data?.filterSpuelung?.filter3 },
                          { label: 'Filter 4', value: selectedHistory.data?.filterSpuelung?.filter4 },
                        ])}
                      </div>
