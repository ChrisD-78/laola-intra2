...
export async function updateTechnikInspection(id: string, partial: Partial<TechnikInspectionRecord>) {
  const response = await fetch('/api/technik', {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ id, ...partial })
  })
  if (!response.ok) throw new Error('Failed to update technik inspection')
  return response.json()
}
...
export async function deleteTechnikInspection(id: string) {
  ...
  if (!response.ok) throw new Error('Failed to delete technik inspection')
  return response.json()
}